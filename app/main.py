...
# Stub BMRDA services — commented until bylaws are extracted
...
@limiter.limit("20/minute")
def water_proximity_check(request: Request, body: Coordinate):
    from app.services.proximity_service import check_water_body_proximity
    return check_water_body_proximity(body.lat, body.lng)
...
# ── BMRDA stub endpoints — commented until bylaws are extracted ──────────────────
# Hoskote, Nelamangala, Kanakapura, Ramanagara, BIAAPA endpoints are disabled.
# Uncomment each block when the respective LPA bylaw JSON is ready.
